...
ScanSet::~ScanSet() {
    for ( auto& scanPtr : scanList ) {
        if ( scanPtr != nullptr ) delete scanPtr;
    }
}
...
TGraphErrors* ScanSet::makeGraphs() {
    ...
    graphSigma = new TGraphErrors( x.size(), &( x[0] ), &( sigma[0] ), 0, &( deltaSigma[0] ) );
    graphSigma2 = new TGraphErrors( x.size(), &( x[0] ), &( sigma2[0] ), 0, &( deltaSigma2[0] ) );
...
TF1* ScanSet::getUpperEdge( double xMin, double xMax, Color_t col ) {
    ...
    upperEdge->SetParameter( 3, graphSigma2->GetFunction( "sigma2" )->GetParError( 1 ) );
